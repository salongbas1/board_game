...
function applyNormalAttackToEditForm(na) {
  ...
  document.getElementById('dndNormalATKRange').value = cfg.range || 0;
}
...
function renderDndNormalAttackInfo() {
  ...
  el.innerHTML = `⚔️ ${escapeHtml(nameTag)} (${statTag})<br>🎯 โจมตี: <b>1d20 ${modStr} ${equipAtk ? `+ ${equipAtk} อุปกรณ์` : ''}</b>📏 ระยะโจมตี: <b>${na.range || 0}</b><br>💥 ดาเมจ: <b>${dmgCount}d${dmgDie} ${dmgModStr} ${equipAtk ? `+ ${equipAtk} อุปกรณ์` : ''}</b>`;
}
...
document.getElementById('dndDmEditSaveBtn').onclick = (ev) => {
  ...
  send({
    ...
    updates: {
      ...
      range: document.getElementById('dndNormalATKRange').value,
    },
  });
  {
